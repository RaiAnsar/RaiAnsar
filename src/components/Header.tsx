...
function AvailabilityBadge() {
  let [status, setStatus] = useState<'Available' | 'Away' | 'Offline'>('Available')

  useEffect(() => {
    async function fetchStatus() {
      try {
        let res = await fetch('/api/status.json')
  ...
  return (
    <div className="pointer-events-auto hidden items-center gap-2 rounded-full bg-white/90 px-3 py-2 text-xs font-medium text-zinc-800 shadow-lg ring-1 shadow-zinc-800/5 ring-zinc-900/5 backdrop-blur-sm sm:flex dark:bg-zinc-800/90 dark:text-zinc-200 dark:ring-white/10">
